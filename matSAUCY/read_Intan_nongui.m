function [] = read_Intan_nongui( filename )
%Reads file from Intan with extension *.rhd and outputs all results
tic;
fid = fopen(filename, 'r');
id_name=strfind(filename,'.rhd');
savename=strcat(filename(1:id_name-1),'.mat');
...
if (data_present)
    ...
    % %%% code to save amplifier_data for use with SpikeDetekt
    % added by David Nicholson 11/9/13 %%%
    % convert amplifier_data to int16
    ampDataInt16 = amplifier_data - 32768;
    ampDataInt16 = int16(ampDataInt16);
    % now convert timestamps into int16 chunks
    % this will be a problem if there are ever more than 32767 samples
    % in a file ... not clear on why spikeDetekt would use an int16
    % timestamp
    timeAmpInt16 = int16(t_amplifier-t_amplifier(1));
...
% Save variables.
save(savename,'notes','frequency_parameters','ampDataInt16','timeAmpInt16');
if (num_amplifier_channels > 0) && (data_present)
    save(savename,'amplifier_channels','amplifier_data','t_amplifier','spike_triggers','-append')
end
if (num_aux_input_channels > 0) && (data_present)
    save(savename,'aux_input_channels','aux_input_data','t_aux_input','-append')
end
if (num_supply_voltage_channels > 0) && (data_present)
    save(savename,'supply_voltage_channels','supply_voltage_data','t_supply_voltage','-append')
end
if (num_board_adc_channels > 0) && (data_present)
    save(savename,'board_adc_channels','board_adc_data','t_board_adc','-append')
end
if (num_board_dig_in_channels > 0) && (data_present)
    save(savename,'board_dig_in_channels','board_dig_in_data','t_dig_in','-append')
end
if (num_board_dig_out_channels > 0) && (data_present)
    save(savename,'board_dig_out_channels','board_dig_out_data','t_dig_out','-append')
end
if (num_temp_sensor_channels > 0) && (data_present)
    save(savename,'temp_sensor_data','t_temp_sensor','-append')
end

fprintf(1, 'Done!  Elapsed time: %0.1f seconds\n', toc);
fprintf(1, 'Extracted data are now saved as %s.\n',savename);
fprintf(1, '\n');
